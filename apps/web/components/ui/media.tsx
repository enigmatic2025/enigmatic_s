...
type MediaProps = {
  ...
  priority?: boolean;
  /** Soft bottom fade so bright photos settle into the black ground. */
  scrim?: boolean;
...
export function Media({ src, alt, sizes = "wide", priority, scrim, caption, className, imgClassName }: MediaProps) {
  const frame = (
    <div className={cn("relative overflow-hidden rounded-md bg-surface-1", className)}>
      ...
      />
      {scrim && <div aria-hidden className="pointer-events-none absolute inset-0 bg-linear-to-t from-background/50 via-background/0 to-background/0" />}
