...
import { TimelineItem, TimelineStep } from "@/components/flow-detail/timeline-item";
import { ActionExecutionPanel } from "@/components/flow-detail/action-execution-panel";
import { CommentSection } from "@/components/flow-detail/comment-section";
...
import { getFlowData, MOCK_FILES } from "@/components/flow-detail/flow-data";

export function DriversAtRiskDemo() {
    // Hardcoded for "Drivers at Risk"
    const flowId = "AF-2024-007";
    ...
    useEffect(() => {
        ...
        if (flow) {
            ...
            // Select the first actionable step for demo purposes if none selected
            // Or just leave it null to encourage interaction
            // Let's select the "Initial Outreach Call" (id: 3) as it's assigned to Sam Tran
            const initialStep = newSteps.find(s => s.id === "3");
            if (initialStep) {
    ...
    return (
        <div className="flex h-full bg-background border rounded-lg overflow-hidden shadow-sm">
                    ...
                    <div className="p-4 pb-4">
                        <div className="flex items-center justify-between mb-3">
                            <Button
                                variant="ghost"
                                size="sm"
                                className="gap-1.5 -ml-2 text-muted-foreground hover:text-foreground cursor-not-allowed opacity-70"
                            >
